...
public class DealerHand : Hand {
    ...
    /// <summary>
    /// ディーラーのハンドが終了しているかどうかを更新する。
    /// </summary>
    /// <remarks>ディーラーのハンドが終了している場合は<c>isFinished</c>を<c>true</c>にする。</remarks>
    /// <remarks>ディーラーのハンドが終了していない場合は<c>isFinished</c>を<c>false</c>にする。</remarks>
    /// <remarks>ディーラーのハンドが終了しているかどうかは、点数が<c>hitlimit</c>で判断する。</remarks>
    private void UpdateIsFinished() {
        SetIsFinished(this.point > this.hitLimitPoint);
    }
